const Documents = ({ token, API_URL }) => {
  ...
  const [modalData, setModalData] = React.useState({ templates: [], users: [], departments: [] });
  const [form, setForm] = React.useState({ template_id: "", signatories: [], distributions: [] });
  ...
  const fetchModalData = async () => {
    try {
      const [t, u, d] = await Promise.all([
        fetch(`${API_URL}/kb/templates`, { headers: { Authorization: `Bearer ${token}` } }),
      ...
      <div className="w-96 border-r border-slate-200 bg-white flex flex-col">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-slate-800 tracking-tight">Документы</h2>
              <button 
          ...
          <form onSubmit={(e) => { e.preventDefault(); fetchDocs(searchQuery); }} className="relative">
            <input
              type="text"
              value={searchQuery}
        ...
        <div className="flex-grow overflow-y-auto divide-y divide-slate-50">
          {loading ? (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-slate-400 font-medium">Работает ИИ...</p>
            </div>
          ) : (
            docs.map((doc) => (
              <button
              ...
              >
                <div className={`mt-1 w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${doc.title.endsWith(".xlsx") ? "bg-green-50 text-green-600" : "bg-indigo-50 text-indigo-500"}`}>
                  <i className={`fas ${doc.title.endsWith(".xlsx") ? "fa-file-excel" : "fa-file-lines"}`}></i>
                </div>
        ...
        {selectedDoc ? (
          <div className="max-w-3xl w-full bg-white shadow-2xl rounded-2xl p-12 min-h-screen border border-slate-200 relative overflow-hidden">
            <div className="flex justify-between items-start mb-10 pb-6 border-b border-slate-100">
              <div>
                <h2 className="text-3xl font-extrabold text-slate-800 leading-tight">{selectedDoc.title}</h2>
                <p className="text-xs text-slate-400 mt-3 flex items-center gap-2 font-medium">
                  <i className="far fa-calendar"></i> {selectedDoc.payload?.metadata?.date || "08.06.2026"}
                  <span className="opacity-30">|</span>
                  <i className="far fa-user"></i> {selectedDoc.payload?.metadata?.author_mention || "Автор не указан"}
                </p>
              </div>
          ...
          <div className="flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-lg font-bold text-slate-400">Выберите документ для просмотра</p>
          </div>
        )}
      </div>
              ...
              <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                  <div className="bg-indigo-600 p-6 flex justify-between items-center text-white">
                      <div>
                        ...
                        <p className="text-indigo-100 text-[11px] font-medium uppercase tracking-wider mt-1">Новый черновик по шаблону</p>
                      </div>
                      <button onClick={() => setIsModalOpen(false)} className="w-8 h-8 rounded-full hover:bg-white/10 transition flex items-center justify-center">
                  ...
                  <form onSubmit={handleCreate} className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                      {/* Шаблон */}
                      <div>
                          <label className="block text-xs font-bold text-slate-400 uppercase mb-3 tracking-widest ml-1">1. Выбор шаблона</label>
                          <div className="grid grid-cols-1 gap-2">
                            {modalData.templates.map(t => (
                                ...
                                >
                                    <p className={`font-bold text-sm ${form.template_id === t.id ? 'text-indigo-700' : 'text-slate-700'}`}>{t.title}</p>
                                    <p className="text-[10px] text-slate-400 font-medium uppercase mt-1">{t.doc_type}</p>
                                </button>
                            ))}
                          </div>
                      </div>
                      ...
                      {/* Подписанты */}
                      <div>
                          <label className="block text-xs font-bold text-slate-400 uppercase mb-3 tracking-widest ml-1">2. Кто должен подписать?</label>
                          <div className="space-y-2">
                              {modalData.users.filter(u => u.username !== 'GavrilovAA').map(u => (
                                  <label key={u.id} className={`flex items-center p-3 rounded-xl border transition-all cursor-pointer ${form.signatories.includes(u.id) ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                                      <input type="checkbox" checked={form.signatories.includes(u.id)} onChange={() => toggleSignatory(u.id)} className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300" />
                                      <div className="ml-3">
                                        <p className="text-sm font-bold text-slate-700 leading-none">{u.full_name}</p>
                                        <p className="text-[10px] text-slate-400 font-medium mt-1">{u.department?.name || 'Отдел не указан'}</p>
                                      </div>
                                  </label>
                              ))}
                          </div>
                      ...
                      {/* Рассылка по отделам */}
                      <div>
                          <label className="block text-xs font-bold text-slate-400 uppercase mb-3 tracking-widest ml-1">3. Кому в рассылку? (Отделы)</label>
                  ...
                  <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                      <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3.5 text-sm font-bold text-slate-400 hover:text-slate-600 transition">Отмена</button>
                      <button onClick={handleCreate} disabled={creating || !form.template_id} className="flex-[2] py-3.5 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 disabled:shadow-none transition flex items-center justify-center gap-2">
                          {creating ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <i className="fas fa-check text-xs"></i>}
